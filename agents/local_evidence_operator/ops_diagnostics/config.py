"""Configuration loader for the ops_diagnostics feature."""
...
def _load_toml() -> dict:
    candidates = [
        Path(os.environ.get("OPS_DIAGNOSTICS_CONFIG", "")),
        Path.cwd() / "config.toml",
        Path(__file__).resolve().parent.parent / "config" / "ops_diagnostics.toml",
    ]
...
class ServerSettings(BaseSettings):
    ...
    model_config = {"env_prefix": "OPS_DIAGNOSTICS_SERVER_"}


class SecuritySettings(BaseSettings):
    api_key: str = ""

    model_config = {"env_prefix": "OPS_DIAGNOSTICS_SECURITY_"}
...
class WorkspaceSettings(BaseSettings):
    ...
    model_config = {"env_prefix": "OPS_DIAGNOSTICS_WORKSPACE_"}
