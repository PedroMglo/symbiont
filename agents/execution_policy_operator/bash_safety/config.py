"""Configuration loader for the bash_safety provider."""
...
def _load_toml() -> dict:
    candidates = [
        Path(os.environ.get("BASH_SAFETY_CONFIG", "")),
        Path.cwd() / "config.toml",
        Path(__file__).resolve().parent.parent / "config" / "bash_safety.toml",
    ]
...
class ServerSettings(BaseSettings):
    ...
    model_config = {"env_prefix": "BASH_SAFETY_SERVER_"}


class SecuritySettings(BaseSettings):
    api_key: str = ""

    model_config = {"env_prefix": "BASH_SAFETY_SECURITY_"}
...
class WorkspaceSettings(BaseSettings):
    ...
    model_config = {"env_prefix": "BASH_SAFETY_WORKSPACE_"}
