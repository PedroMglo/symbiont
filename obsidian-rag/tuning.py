...
def auto_tune(perf: "PerformanceConfig") -> "PerformanceConfig":
    ...
    result = PerformanceConfig(
        ...
        swap_pause_requires_active_pressure=perf.swap_pause_requires_active_pressure,
        swap_active_growth_gb=perf.swap_active_growth_gb,
        swap_pressure_ram_percent=perf.swap_pressure_ram_percent,
        job_end_memory_cleanup=perf.job_end_memory_cleanup,
        job_end_malloc_trim=perf.job_end_malloc_trim,
        job_end_clear_embedder_cache=perf.job_end_clear_embedder_cache,
        embedding_concurrency=perf.embedding_concurrency,
        embedding_lane_concurrency=perf.embedding_lane_concurrency,
        source_scan_parallel_jobs=perf.source_scan_parallel_jobs,
        resource_pause_max_seconds=perf.resource_pause_max_seconds,
        resource_pause_total_budget_seconds=perf.resource_pause_total_budget_seconds,
        resource_retry_max_attempts=perf.resource_retry_max_attempts,
...
def should_throttle(perf: "PerformanceConfig", data_dir: str | None = None) -> ThrottleAdvice:
    ...
    if action is GovernorAction.ABORT:
        ...
        if snap and snap.swap_percent >= getattr(perf, "abort_swap_percent", 80) and snap.active_swap_pressure:
    ...
    if action is GovernorAction.PAUSE:
        ...
        if snap and snap.swap_percent >= getattr(perf, "pause_swap_percent", 60):
            state = "ativo" if snap.active_swap_pressure else "residual"
            reasons.append(f"Swap {snap.swap_percent:.0f}% {state} ({snap.swap_used_gb:.1f} GB)")
