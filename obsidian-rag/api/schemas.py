...
class AdminReprocessSource(BaseModel):
    path: str = Field(..., min_length=1, max_length=4096, description="User-requested local source path")
    name: str | None = Field(default=None, max_length=120, description="Optional source namespace")
    source_type: Literal["auto", "document", "code", "vault"] = Field(
        default="auto",
        description="Source classification hint. auto detects vault, Git/code, or heterogeneous documents.",
    )
    exclude_patterns: list[str] = Field(
        default_factory=list,
        max_length=100,
        description="Source-relative glob patterns to exclude during runtime ingestion.",
    )


class AdminReprocessOrigin(BaseModel):
    kind: Literal["user_machine", "agent", "feature", "service", "scheduler", "unknown"] = Field(
        default="unknown",
        description="Who initiated the request at the RAG API boundary.",
    )
    name: str | None = Field(default=None, max_length=160, description="Human-readable caller/source name")
    agent: str | None = Field(default=None, max_length=160, description="Agent slug when kind=agent")
    feature: str | None = Field(default=None, max_length=160, description="Feature slug when kind=feature")
    service: str | None = Field(default=None, max_length=160, description="Service slug when kind=service")
    machine: str | None = Field(default=None, max_length=255, description="User machine or host identifier")
    user: str | None = Field(default=None, max_length=160, description="Local user/account identifier")
    trace_id: str | None = Field(default=None, max_length=255, description="Upstream trace/request id")
    reason: str | None = Field(default=None, max_length=500, description="Operator-visible reason for the reprocess")
    metadata: dict[str, str] = Field(
        default_factory=dict,
        max_length=50,
        description="Small string metadata map for caller-specific context.",
    )

    @model_validator(mode="after")
    def _fill_name_from_specific_field(self):
        if not self.name:
            self.name = self.agent or self.feature or self.service or self.user or self.machine
        return self
...
class AdminReprocessRequest(BaseModel):
    target: str = Field(
        default="all",
        pattern="^(local|sources|graph|cag|all)$",
        description="Reprocess target: local, sources, graph, cag, or all",
    )
    ...
    sources: list[AdminReprocessSource] = Field(
        default_factory=list,
        max_length=20,
        description="Optional runtime sources requested by the user for this reprocess job",
    )
    origin: AdminReprocessOrigin = Field(
        default_factory=AdminReprocessOrigin,
        description="Normalized caller identity and request provenance.",
    )
...
class AdminJobsResponse(BaseModel):
    jobs: list[AdminJobStatusResponse]
    total: int
    active: int
