...
_RAG_PERFORMANCE_DEFAULTS = {
    ...
    "swap_active_growth_gb": 0.25,
    "swap_pressure_ram_percent": 70,
    "job_end_memory_cleanup": True,
    "job_end_malloc_trim": True,
    "job_end_clear_embedder_cache": True,
...
def _auto_discover_secrets():
    ...
    mappings = {
        ...
        "AI_RESOURCE_GOVERNOR_TOKEN_FILE": "internal_api_key",  # nosec B105 - secret filename, not secret value.
    }
...
class PerformanceConfig:
    ...
    swap_pause_requires_active_pressure: bool = True
    swap_active_growth_gb: float = 0.25
    swap_pressure_ram_percent: int = 70
    job_end_memory_cleanup: bool = True
    job_end_malloc_trim: bool = True
    job_end_clear_embedder_cache: bool = True
    # --- Embedding parallelism ---
    embedding_concurrency: int = 1       # concurrent embedding threads (max 3 for local Ollama)
    embedding_lane_concurrency: int = 1  # global embedding lane concurrency across child source jobs
    source_scan_parallel_jobs: int = 1   # child source scan/parse parallelism; embedding/write lanes are separate
    resource_pause_max_seconds: int = 120
    resource_pause_total_budget_seconds: int = 600
    resource_retry_max_attempts: int = 12
    # --- Manifest batching ---
...
def load_settings() -> Settings:
    ...
    performance = PerformanceConfig(
        ...
        swap_pause_requires_active_pressure=_env_override(
            "performance",
            "swap_pause_requires_active_pressure",
            pf.get(
                "swap_pause_requires_active_pressure",
                _RAG_PERFORMANCE_DEFAULTS["swap_pause_requires_active_pressure"],
            ),
        ),
        swap_active_growth_gb=_env_override(
            "performance",
            "swap_active_growth_gb",
            pf.get("swap_active_growth_gb", _RAG_PERFORMANCE_DEFAULTS["swap_active_growth_gb"]),
        ),
        swap_pressure_ram_percent=_env_override(
            "performance",
            "swap_pressure_ram_percent",
            pf.get("swap_pressure_ram_percent", _RAG_PERFORMANCE_DEFAULTS["swap_pressure_ram_percent"]),
        ),
        job_end_memory_cleanup=_env_override(
            "performance",
            "job_end_memory_cleanup",
            pf.get("job_end_memory_cleanup", _RAG_PERFORMANCE_DEFAULTS["job_end_memory_cleanup"]),
        ),
        job_end_malloc_trim=_env_override(
            "performance",
            "job_end_malloc_trim",
            pf.get("job_end_malloc_trim", _RAG_PERFORMANCE_DEFAULTS["job_end_malloc_trim"]),
        ),
        job_end_clear_embedder_cache=_env_override(
            "performance",
            "job_end_clear_embedder_cache",
            pf.get(
                "job_end_clear_embedder_cache",
                _RAG_PERFORMANCE_DEFAULTS["job_end_clear_embedder_cache"],
            ),
        ),
        embedding_concurrency=_env_override("performance", "embedding_concurrency", pf.get("embedding_concurrency", 1)),
        embedding_lane_concurrency=_env_override(
            "performance",
            "embedding_lane_concurrency",
            pf.get("embedding_lane_concurrency", _RAG_PERFORMANCE_DEFAULTS["embedding_lane_concurrency"]),
        ),
        source_scan_parallel_jobs=_env_override(
            "performance",
            "source_scan_parallel_jobs",
            pf.get("source_scan_parallel_jobs", _RAG_PERFORMANCE_DEFAULTS["source_scan_parallel_jobs"]),
        ),
        resource_pause_max_seconds=_env_override(
            "performance",
            "resource_pause_max_seconds",
            pf.get("resource_pause_max_seconds", _RAG_PERFORMANCE_DEFAULTS["resource_pause_max_seconds"]),
        ),
        resource_pause_total_budget_seconds=_env_override(
            "performance",
            "resource_pause_total_budget_seconds",
            pf.get(
                "resource_pause_total_budget_seconds",
                _RAG_PERFORMANCE_DEFAULTS["resource_pause_total_budget_seconds"],
            ),
        ),
        resource_retry_max_attempts=_env_override(
            "performance",
            "resource_retry_max_attempts",
            pf.get("resource_retry_max_attempts", _RAG_PERFORMANCE_DEFAULTS["resource_retry_max_attempts"]),
        ),
