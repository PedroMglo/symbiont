...
def _matches_exclude(rel_path: str, name: str, extra_patterns: tuple[str, ...] = ()) -> bool:
    for pattern in (*settings.sync.exclude_patterns, *extra_patterns):
        if fnmatch.fnmatch(rel_path, pattern) or fnmatch.fnmatch(name, pattern):
            return True
    return False


def iter_document_files(root: Path | str, *, exclude_patterns: tuple[str, ...] = ()) -> Iterator[Path]:
    ...
    for path in sorted(root_path.rglob("*")):
        ...
        if suffix not in _DOCUMENT_EXTENSIONS:
            continue
        if suffix not in _AUDIO_EXTENSIONS and _matches_exclude(rel_text, path.name, exclude_patterns):
            continue
        yield path
...
def _try_audio_transcription(path: Path) -> str:
    from integrations.external_services import (
        ...
        request_background_lease_with_wait,
    )
    ...
    lease = request_background_lease_with_wait(
        ...
        max_wait_seconds=max(30, timeout),
    )
    try:
