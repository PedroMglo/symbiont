...
from obsidian_rag.retrieval.rag import (
    _get_store,
    build_rag_context_async,
    should_use_rag,
)
from obsidian_rag.workflows.job_store import default_admin_job_store
...
async def lifespan(app: FastAPI):
    ...
    if settings.observability.enabled:
        from obsidian_rag.observability import start as obs_start
        obs_start(
    ...
    if settings.observability.enabled:
        from obsidian_rag.observability import stop as obs_stop
        obs_stop()
...
def _admin_reprocess_running() -> bool:
    ...
    with _admin_jobs_lock:
        return any(job.get("status") in {"queued", "running", "submitted"} for job in _admin_jobs.values())
...
# === Dashboard router ===
from obsidian_rag.observability.dashboard import router as dashboard_router  # noqa: E402
...
def _query_store(
    ...
    *,
    ...
    from obsidian_rag.retrieval.rag import _get_sparse_query
...
def _hydrate_admin_jobs() -> None:
    ...
    with _admin_jobs_lock:
        for job_id, job in stored.items():
            _admin_jobs.setdefault(job_id, dict(job))
...
def _execute_or_submit_admin_reprocess(
    ...
    *,
    ...
    payload = {"target": target, "force": force, "vault": vault}
    result_prefix = extra_result or {}
    if settings.workflows.backend == "temporal":
        from obsidian_rag.workflows.temporal_client import start_reprocess_workflow
    ...
    from obsidian_rag.workflows.reprocess import execute_reprocess_target

    result = execute_reprocess_target(target, force=force, vault=vault)
    _set_admin_job(
        job_id,
        status="completed",
...
def _run_admin_reprocess(job_id: str, request: AdminReprocessRequest) -> None:
...
def admin_reprocess(request: AdminReprocessRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(_run_admin_reprocess, job_id, request)
...
def admin_job_status(job_id: str):
    ...
    if job.get("status") == "submitted" and result.get("backend") == "temporal":
        try:
            from obsidian_rag.workflows.temporal_client import describe_reprocess_workflow
...
def indexing_status():
    ...
    try:
        ...
        from obsidian_rag.pipeline.sync import _compute_config_version
...
def retrieval_status(hours: int = 24, recent: int = 20):
    ...
    from obsidian_rag.retrieval.audit import read_recent, summary_stats
    from obsidian_rag.retrieval.sparse import bm25_status
