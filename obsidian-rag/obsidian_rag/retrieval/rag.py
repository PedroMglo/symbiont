...
from obsidian_rag.metadata import (
    NOTE_TITLE,
    REPO_NAME,
    SOURCE_PATH,
)
...
from obsidian_rag.retrieval.tokenization import STOP_WORDS, extract_keywords
...
def _maybe_rerank(chunks: list[ScoredChunk], query: str, trace: QueryTrace) -> list[ScoredChunk]:
    ...
    try:
        ...
        try:
            from obsidian_rag.retrieval.cross_encoder_reranker import (
                is_available,
...
def _inject_cag_context(decision_mode: str, query: str, trace: QueryTrace) -> str:
    ...
    try:
        ...
        from obsidian_rag.cag.packs import get_relevant_packs
...
def _audit_retrieval(
    *,
    ...
    try:
        from obsidian_rag.retrieval.audit import log_retrieval
...
def build_rag_context(
    ...
    *,
    ...
    if intent.use_graph and not code_relevant and not intent.use_code:
        try:
            from obsidian_rag.retrieval.graph_context import build_graph_query_context
...
def _emit_retrieval_event(trace: QueryTrace, query: str, t0: float, cfg) -> None:
    ...
    if not is_enabled():
        return
    from obsidian_rag.observability import EventName, RAGEvent, query_hash
...
def should_use_rag(model: str) -> bool:
    ...
    from obsidian_rag.registry import is_rag_capable
