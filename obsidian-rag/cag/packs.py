...
from cag.freshness import compute_config_version, compute_source_hash
...
def gen_project_architecture(store: PackStore, scope: str) -> str | None:
    ...
    from rag_config import PROJECT_ROOT
...
def gen_security_exclusions(store: PackStore, scope: str) -> str | None:
    ...
    from rag_config import _DEFAULT_EXCLUDE_PATTERNS, settings
...
def gen_pending_tasks(store: PackStore, scope: str) -> str | None:
    ...
    tasks: list[str] = []
    for pyfile in iter_repo_files(src):
        if pyfile.suffix.lower() != ".py":
            continue
        try:
