...
class IngestManifest:
    ...
    def delete_source_types(self, source_types: tuple[str, ...]) -> dict[str, int]:
        """Remove all manifest state for the given source types."""
        clean_types = tuple(dict.fromkeys(st for st in source_types if st))
        if not clean_types:
        ...
        with self._tx() as cur:
            cur.execute(
                _sql("fstring_delete_chunks_source_types.sql").format(placeholders),  # noqa: S608
                clean_types,
            )
            chunks_deleted = cur.rowcount if cur.rowcount >= 0 else 0
            cur.execute(
                _sql("fstring_delete_files_source_types.sql").format(placeholders),  # noqa: S608
                clean_types,
            )
            files_deleted = cur.rowcount if cur.rowcount >= 0 else 0
        return {"files": files_deleted, "chunks": chunks_deleted}

    def delete_run_history(self) -> int:
        """Remove ingest run history from the manifest."""
        with self._tx() as cur:
            cur.execute(_sql("execute_delete_ingest_runs.sql"))
