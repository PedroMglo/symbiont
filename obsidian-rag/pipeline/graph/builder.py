...
_GRAPHIFY_PROC_LOCK = threading.Lock()
_GRAPHIFY_PROCS: dict[int, subprocess.Popen] = {}


def _register_graphify_proc(proc: subprocess.Popen) -> None:
    with _GRAPHIFY_PROC_LOCK:
        _GRAPHIFY_PROCS[int(proc.pid)] = proc


def _unregister_graphify_proc(proc: subprocess.Popen) -> None:
    with _GRAPHIFY_PROC_LOCK:
        _GRAPHIFY_PROCS.pop(int(proc.pid), None)


def cancel_active_graphify_processes(*, grace_seconds: float = 5.0) -> dict[str, object]:
    """Terminate Graphify subprocesses owned by this RAG API process."""
    with _GRAPHIFY_PROC_LOCK:
        procs = list(_GRAPHIFY_PROCS.values())
    ...
    for proc in procs:
        ...
        if proc.poll() is not None:
            ...
            _unregister_graphify_proc(proc)
            continue
        try:
            proc.terminate()
...
def _configured_graph_roots() -> list[Path]:
    """Return graphable roots from configured sources.
    ...
    for raw_root in tuple(settings.repos.paths):
...
def build_graph(
    ...
    *,
    ...
    if cancel_event is not None and cancel_event.is_set():
        log.info("[Graphify] cancelado antes de '%s'.", repo_path.name)
        return False
    ...
    from pipeline.graph.checkpoints import (
        build_fingerprint,
        checkpoint_completed_for_fingerprint,
        estimate_work,
        load_checkpoint,
        write_checkpoint,
    )
    ...
    fingerprint = build_fingerprint(
        graph_input_path,
        graphable_extensions=_GRAPHABLE_EXTENSIONS,
        skip_patterns=settings.graphify.skip_patterns,
        size_limit=_file_size_limit_for_graph(graph_input_path),
    )
    dry_run_estimate = estimate_work(fingerprint, force=force)
    if (
        graph_json.exists()
        and not force
        and checkpoint_completed_for_fingerprint(load_checkpoint(checkpoint_json), fingerprint)
    ):
        log.info("[Graphify] Checkpoint completo para '%s' — skipping.", repo_path.name)
        print(f"  [graphify] {repo_path.name}: checkpoint completo — skip")
        return _sync_graph_query_backend(repo_path, graph_json)
    if force:
        log.info("[Graphify] dry-run force estimate for '%s': %s", repo_path.name, dry_run_estimate)
    ...
    if force:
        ...
        mode = "build inicial"
    checkpoint_stage = "update" if use_update else "extract"
                ...
                while True:
                    try:
                        ...
                        if cancel_event.is_set():
                            ...
                            log.info("[Graphify] cancelado durante '%s'.", repo_path.name)
                            return False
...
def get_prefilter_stats(repo_path: Path) -> dict[str, int]:
    ...
    from pipeline.graph.prefilter import filter_files_for_llm
...
def build_graphs(*, force: bool = False, cancel_event: threading.Event | None = None) -> None:
    ...
    if not graph_roots:
        ...
        return
    if cancel_event is not None and cancel_event.is_set():
        log.info("[Graphify] cancelado antes de processar roots.")
        return
    ...
    def _build_one(repo_path: str) -> bool:
        ...
        from integrations.resource_governor_client import lease_context
        ...
        with lease_context(
            ...
            if not lease.granted:
                ...
                return False
            return build_graph(repo_path, force=force, cancel_event=cancel_event)
