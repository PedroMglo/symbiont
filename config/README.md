...
temperature, token cap and timeout fallbacks are generated by the resolver and
...
fixed project templates or fallback content.
...
```bash
PYTHONPATH=storage_guardian/src python -m storage_guardian.cli --config config/storage_guardian.yaml reconcile-structure
PYTHONPATH=storage_guardian/src python -m storage_guardian.cli --config config/storage_guardian.yaml reconcile-structure --apply
PYTHONPATH=storage_guardian/src python -m storage_guardian.cli --config config/storage_guardian.yaml sync-pending
```
