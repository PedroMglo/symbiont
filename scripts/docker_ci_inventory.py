...
def _external_entries(compose: dict[str, Any], *, project_image_repositories: set[str] | None = None) -> list[dict[str, Any]]:
    project_image_repositories = project_image_repositories or set()
    ...
    for service_name, service in sorted((compose.get("services") or {}).items()):
        ...
        if not image:
            continue
        if _image_repository(image) in project_image_repositories:
            continue
...
def build_ci_inventory(
    ...
    *,
    ...
    project_image_repositories = {_image_repository(str(item.get("tag") or "")) for item in build_images}
    external_images = _external_entries(compose, project_image_repositories=project_image_repositories)
